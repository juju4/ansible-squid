...
describe command("curl -v -x http://localhost:#{proxy_port} http://2016.eicar.org/download/eicar.com.txt"), :if => os[:family] == 'ubuntu' || os[:family] == 'debian' do
  its(:stdout) { should match /<title>DansGuardian - Access Denied<\/title>/ }
...
describe file('/var/log/dansguardian/access.log'), :if => os[:family] == 'ubuntu' || os[:family] == 'debian' do
  ...
  its(:content) { should match /http:\/\/2016.eicar.org\/download\/eicar.com.txt \*INFECTED\* \*DENIED\* Virus or bad content detected. Win.Test.EICAR_HDB-1/ }
end
